...
export function ResponseViewer({
  ...
  if (!response && !error) {
    return (
      <Card className="bg-card-30 border-card-foreground">
        <CardContent className="text-primary p-4">No response yet</CardContent>
      </Card>
    );
  }
      ...
      <Card className="border-transparent bg-transparent shadow-none">
        <CardContent className="flex flex-col gap-4">
          <div className="flex items-center gap-2">
            <label className="w-16 text-sm font-medium">Status</label>
            <Input value={response?.status} readOnly />
          </div>
          <div className="flex items-center gap-2">
            <label className="w-16 text-sm font-medium">Error</label>
