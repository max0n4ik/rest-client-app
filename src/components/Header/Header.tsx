...
const Header = (): JSX.Element => {
  ...
  return (
    <>
      {' '}
      <header
        className={`sticky top-0 z-50 flex h-14 min-h-14 w-full items-center justify-between transition-colors duration-300 md:justify-center ${scrolled ? 'bg-secondary text-white shadow-lg' : 'bg-background text-foreground'}`}>
        {' '}
      ...
      {menuOpen && (
        <div
          className={`${scrolled ? 'bg-secondary text-white shadow-lg' : 'bg-background text-foreground'} sticky top-14 z-50 block w-full text-xl font-semibold`}>
          {' '}
